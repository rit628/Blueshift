...
std::any DepGraph::visit(AstNode::Function::Oblock& ast){
...
std::any DepGraph::visit(AstNode::Statement::Declaration& ast) {
    if(this->setupCtx.inSetup){ 
        ...
        if(valStatement.has_value()){
            auto container = valStatement.value()->accept(*this); 
            if(container.type() == typeid(std::string)){
                std::string constructor = std::any_cast<std::string>(container); 
...
std::any DepGraph::visit(AstNode::Statement::Expression &ast){
...
std::any DepGraph::visit(AstNode::Expression::Function& ast) {
    if(this->setupCtx.inSetup){
        ...
        for(auto& dev : argList){
            auto devArg = dev->accept(*this); 
            if(devArg.type() == typeid(std::string)){
                auto bindDev = std::any_cast<std::string>(devArg); 
...
std::any DepGraph::visit(AstNode::Statement::If& ast) {
...
std::any DepGraph::visit(AstNode::Statement::For& ast) {
...
std::any DepGraph::visit(AstNode::Statement::While& ast) {
    ast.getCondition()->accept(*this); 
    for(auto &sm : ast.getBlock()){
        sm->accept(*this); 
    }
...
std::any DepGraph::visit(AstNode::Expression::Group& ast) {
    ast.getExpression()->accept(*this); 
    return true; 
}


std::any DepGraph::visit(AstNode::Expression::Unary& ast){
...
std::any DepGraph::visit(AstNode::Function::Procedure& ast) {
...
std::any DepGraph::visit(AstNode::Statement::Return& ast) {
    
}

std::any DepGraph::visit(AstNode::Statement::Continue& ast) {

}

std::any DepGraph::visit(AstNode::Statement::Break& ast) {
...
std::any DepGraph::visit(AstNode::Expression::Method& ast) {

}


std::any DepGraph::visit(AstNode::Expression::List& ast) {

}

std::any DepGraph::visit(AstNode::Expression::Set& ast) {
    
}

std::any DepGraph::visit(AstNode::Expression::Map& ast) {

}

std::any DepGraph::visit(AstNode::Specifier::Type& ast) {
